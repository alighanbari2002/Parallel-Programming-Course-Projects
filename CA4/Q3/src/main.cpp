...
int main()
{
    ...
    size_t num_threads = std::min<size_t>(4, std::thread::hardware_concurrency());

    long long elapsed_time_serial = calculate_absolute_difference_serial(first_image, second_image);
	long long elapsed_time_parallel = calculate_absolute_difference_parallel(first_image, second_image, num_threads);
